...
class Http2Stream internal constructor(
  ...
  inner class FramingSource internal constructor(
    ...
    @Throws(IOException::class)
    override fun close() {
      var responseHeaderLimitFailure: ResponseHeaderLimitException? = null
      try {
        // Match HTTP/1's bounded close drain so trailers arriving with the final DATA frames are
        // parsed and policy failures can reach the response consumer. Unfinished streams are still
        // cancelled after this short opportunity to complete.
        discard(ExchangeCodec.DISCARD_STREAM_TIMEOUT_MILLIS, TimeUnit.MILLISECONDS)
      } catch (e: ResponseHeaderLimitException) {
        responseHeaderLimitFailure = e
      }
      ...
      responseHeaderLimitFailure?.let { throw it }
    }
  }
