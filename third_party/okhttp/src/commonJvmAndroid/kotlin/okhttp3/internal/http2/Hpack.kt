...
object Hpack {
  ...
  class Reader
    ...
    constructor(
      ...
      private data class ResponseHeaderBudget(
        var fieldCount: Int = 0,
        var metadataByteCount: Int = 0,
        var pseudoFieldCount: Int = 0,
      )
      private val responseHeaderBudgets = mutableMapOf<Int, ResponseHeaderBudget>()
      private var activeResponseHeaderBudget: ResponseHeaderBudget? = null
      private var blockPseudoFieldCount = 0
      ...
      fun getAndResetHeaderList(
        ...
        activeResponseHeaderBudget = null
        if (endStream) responseHeaderBudgets.remove(streamId)
        return result
      }

      /** Forget response accounting when a stream is reset before END_STREAM. */
      @Synchronized
      fun clearResponseHeaderBudget(streamId: Int) {
        responseHeaderBudgets.remove(streamId)
      }
      ...
      @Synchronized
      fun readHeaders(streamId: Int = 0) {
        activeResponseHeaderBudget = responseHeaderBudgets.getOrPut(streamId) { ResponseHeaderBudget() }
        blockPseudoFieldCount = 0
      ...
      private fun readLiteralHeaderWithoutIndexingIndexedName(index: Int) {
        ...
        val value = readByteString(RESPONSE_HEADER_VALUE_BYTE_LIMIT, ResponseHeaderLimitKind.VALUE_BYTES)
        addHeader(Header(name, value))
      }

      @Throws(IOException::class)
      private fun readLiteralHeaderWithoutIndexingNewName() {
        val name = checkLowercase(readByteString(RESPONSE_HEADER_AGGREGATE_BYTE_LIMIT, ResponseHeaderLimitKind.AGGREGATE_BYTES))
        val value = readByteString(RESPONSE_HEADER_VALUE_BYTE_LIMIT, ResponseHeaderLimitKind.VALUE_BYTES)
        addHeader(Header(name, value))
      }
      ...
      private fun readByteString(
        maxDecodedByteCount: Int,
        limitKind: ResponseHeaderLimitKind,
      ): ByteString {
        ...
        // For uncompressed strings this is the decoded size, so reject before allocating it.
        if (!huffmanDecode && length > maxDecodedByteCount) {
          throw ResponseHeaderLimitException(limitKind, maxDecodedByteCount)
        }
        ...
        return if (huffmanDecode) {
          ...
          Huffman.decode(source, length, decodeBuffer, maxDecodedByteCount, limitKind)
      ...
      private fun addHeader(header: Header) {
        ...
        val valueByteCount = header.value.utf8().encodeUtf8().size
        if (valueByteCount > RESPONSE_HEADER_VALUE_BYTE_LIMIT) throw responseHeaderValueLimitExceeded()

        // Pseudo-fields have no HTTP/1 field equivalent. Keep them outside the cross-protocol
        // response metadata budget, but cap their occurrences per block so repeated pseudo names
        // cannot bypass the retained-header bound. Five covers an extended CONNECT request.
        val isPseudoField = header.name.size > 0 && header.name[0] == ':'.code.toByte()
        if (isPseudoField) {
          blockPseudoFieldCount++
          if (blockPseudoFieldCount > MAX_PSEUDO_FIELD_COUNT) {
            throw ResponseHeaderLimitException(ResponseHeaderLimitKind.PSEUDO_FIELD_COUNT, MAX_PSEUDO_FIELD_COUNT)
          }
          val budget = activeResponseHeaderBudget ?: responseHeaderBudgets.getOrPut(0) { ResponseHeaderBudget() }
          budget.pseudoFieldCount++
          if (budget.pseudoFieldCount > RESPONSE_HEADER_FIELD_LIMIT) {
            throw ResponseHeaderLimitException(ResponseHeaderLimitKind.PSEUDO_FIELD_COUNT, RESPONSE_HEADER_FIELD_LIMIT)
          }
        } else {
          val budget = activeResponseHeaderBudget ?: responseHeaderBudgets.getOrPut(0) { ResponseHeaderBudget() }
          if (budget.fieldCount + 1 > RESPONSE_HEADER_FIELD_LIMIT) {
            throw ResponseHeaderLimitException(ResponseHeaderLimitKind.FIELD_COUNT, RESPONSE_HEADER_FIELD_LIMIT)
          }
          val nameByteCount = header.name.utf8().encodeUtf8().size
          val fieldByteCount = nameByteCount + valueByteCount + RESPONSE_HEADER_FIELD_OVERHEAD_BYTES
          if (budget.metadataByteCount + fieldByteCount > RESPONSE_HEADER_AGGREGATE_BYTE_LIMIT) {
            throw responseHeaderAggregateLimitExceeded()
          }
          budget.fieldCount++
          budget.metadataByteCount += fieldByteCount
        }
      ...
      private companion object {
        const val MAX_PSEUDO_FIELD_COUNT = 5
      }
    }
