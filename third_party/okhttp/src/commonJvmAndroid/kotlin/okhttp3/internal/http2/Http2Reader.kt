...
class Http2Reader(
  ...
  internal fun clearResponseHeaderBudget(streamId: Int) {
    hpackReader.clearResponseHeaderBudget(streamId)
  }
  ...
  private fun readHeaders(
    ...
    val headerBlock = readHeaderBlock(headerBlockLength, padding, flags, streamId, endStream)
  ...
  private fun readHeaderBlock(
    ...
    hpackReader.readHeaders(streamId)
    return hpackReader.getAndResetHeaderList(streamId, endStream)
  }
  ...
  private fun readData(
    ...
    if (inFinished) hpackReader.clearResponseHeaderBudget(streamId)
  }
  ...
  private fun readPushPromise(
    ...
    // PUSH_PROMISE carries the pushed request headers, not response metadata. Do not retain a
    // response-budget entry between this block and the pushed response (which may be cancelled).
    val headerBlock = readHeaderBlock(headerBlockLength, padding, flags, promisedStreamId, endStream = true)
