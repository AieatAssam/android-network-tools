...
class LiteralDestinationClassifierTest {
    @Test
    fun `hostname target records the system resolver without resolving the hostname`() {
        val target = LiteralDestinationClassifier.target("example.com")

        assertEquals(OperationTargetClass.UNKNOWN, target.targetClass)
        assertEquals(ResolverClass.SYSTEM_DEFAULT, target.resolverClass)
    }
