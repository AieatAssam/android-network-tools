...
class OperationAvailabilityTest {
    ...
    fun `missing route permission and policy are definitive denials`() {
        ...
        assertAll(cases.map { case ->
            org.junit.jupiter.api.function.Executable {
                val actual = OperationAvailability.classify(
                    target = target,
                    status = status,
        ...
        val internetOperation = OperationAvailability.classify(
        ...
        val scopedMulticastTarget = OperationTarget(
            LiteralDestinationClassifier.classify("ff02::1%wlan0"),
        )
        val multicastPermissionDenied = OperationAvailability.classify(
