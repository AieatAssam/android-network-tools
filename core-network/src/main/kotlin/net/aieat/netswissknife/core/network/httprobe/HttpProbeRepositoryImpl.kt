...
class HttpProbeRepositoryImpl internal constructor(
    ...
    private suspend fun executeRequest(
        ...
        repeat(MAX_REDIRECTS + 1) { attempt ->
            ...
            val remainingTimeoutMs = currentCoroutineContext()[OperationResourcesContext]
                ...
                ?.coerceAtLeast(1L)?.toInt() ?: request.timeoutMs
            val headers = request.headers.filter { (key, _) ->
                forwardCustomHeaders && (currentMethod.supportsBody || key.lowercase() !in BODY_HEADERS)
            }
            val exchange = engine.newCall(
                HttpEngineRequest(
            ...
            try {
                ...
                val redirect = if (request.followRedirects) {
                ...
                } else RedirectPolicy.Decision.NotRedirect
                when (redirect) {
                    RedirectPolicy.Decision.NotRedirect -> Unit
                    RedirectPolicy.Decision.MalformedLocation -> return NetworkResult.Error("Malformed redirect URL")
                    RedirectPolicy.Decision.UnsupportedProtocol -> return NetworkResult.Error("Redirected to unsupported protocol")
                    RedirectPolicy.Decision.TooManyRedirects -> return NetworkResult.Error("Too many redirects (max $MAX_REDIRECTS)")
                ...
                val bodyRead = response.body?.use { stream ->
    ...
    private fun readResponseBody(stream: java.io.InputStream, maxBytes: Long, charset: Charset): BodyRead {
    ...
    private data class BodyRead(val text: String?, val bufferedBytes: Long, val truncated: Boolean)

    private class HttpCallLease(private val call: HttpEngineCall) : AutoCloseable {
        private val closed = AtomicBoolean(false)
        override fun close() { if (closed.compareAndSet(false, true)) call.close() }
    }

    private fun declaredContentLength(headers: Map<String, List<String>>): Long? =
        headerValues(headers, "Content-Length")?.singleOrNull()?.trim()?.toLongOrNull()?.takeIf { it >= 0L }
    ...
    private fun responseCharset(headers: Map<String, List<String>>): Charset {
        ...
        val name = match?.groupValues?.getOrNull(1)?.ifBlank { null }
    ...
    private fun header(headers: Map<String, List<String>>, name: String): String? =
        headerValues(headers, name)?.firstOrNull()

    private fun headerValues(headers: Map<String, List<String>>, name: String): List<String>? =
        headers.entries.firstOrNull { it.key.equals(name, ignoreCase = true) }?.value

    private fun HttpTimings.plus(other: HttpTimings): HttpTimings = HttpTimings(
    ...
    private fun add(first: Long?, second: Long?): Long? = when {
        first == null -> second
        second == null -> first
        else -> first + second
    }
